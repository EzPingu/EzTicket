import { Activity, BarChart3, ChevronDown, ChevronLeft, ChevronRight, ClipboardList, Clock3, LayoutDashboard, LifeBuoy, PanelLeft, RefreshCw, Search, Settings2, ShieldCheck, Sparkles, LogOut } from "lucide-react";
...
const navItems: { label: ViewKey; icon: typeof LayoutDashboard }[] = [
  { label: "Dashboard", icon: LayoutDashboard }, { label: "Ticket", icon: LifeBuoy }, { label: "Candidature", icon: ClipboardList }, { label: "Storico", icon: Clock3 }, { label: "Statistiche", icon: BarChart3 }, { label: "Configurazione", icon: Settings2 }, { label: "Audit", icon: ShieldCheck },
];
...
export function Layout({ children, activeView, onNavigate, guilds, guild, detail, me, user, onGuildChange, isRefreshing, onRefresh, onLogout, toast }: Props) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${collapsed ? "sidebar--collapsed" : ""}`}>
      ...
      <div className="nav-section">{!collapsed && <span className="nav-label">Workspace</span>}{navItems.slice(0, 5).map(({ label, icon: Icon }) => <button key={label} className={`nav-item ${activeView === label ? "nav-item--active" : ""}`} onClick={() => onNavigate(label)}><Icon size={18} />{!collapsed && <span>{label}</span>}</button>)}</div>
      <div className="nav-section nav-section--bottom">{!collapsed && <span className="nav-label">Manage</span>}{navItems.slice(5).map(({ label, icon: Icon }) => <button key={label} className={`nav-item ${activeView === label ? "nav-item--active" : ""}`} onClick={() => onNavigate(label)}><Icon size={18} />{!collapsed && <span>{label}</span>}</button>)}</div>
